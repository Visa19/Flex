...
try {
    ...
    echo "Error in RiceEdit File Db Connection: " . $exc->getMessage();
}
?>
            ...
            <main class="main-container">
                ...
                try {
                    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])) {
                        if (isset($_POST['rice_name'], $_POST['rice_price'], $_POST['rice_id'])) {
                            ...
                            try {
                                ...
                                $rice = $rice_detail->updateRiceDetails($con, $rice_id);
                                if ($rice) {
                        ...
                        if (isset($_GET['id'])) {
                            ...
                            $rice_detail = $rice_obj->getRiceDetailById($con, $rice_id);

                            if ($rice_detail) {
                                ?>
                                <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data">
                                    <div id="cardStyle" class="card mx-auto shadow p-3 mb-5 rounded" style="width: 50%;">
                                        <div class="card-header text-center"><h4 style="color: #E88F2A;">EDIT RICE DETAILS</h4></div>
                                        <hr>
                                        <div class="card-body">
                                            <div class="card-body">
                                                <div class="form-group my-2">
                                                    <input name="rice_id" type="hidden" id="rice_id" class="form-control" value="<?php echo $rice_id; ?>" required/>  
                                                </div>
                                                <div class="form-group my-2">
                                                    <label for="rice_name">Rice Name :</label>
                                                    <input style="background-color: black; color: white;" name="rice_name" type="text" id="rice_name" class="form-control" value="<?php echo $rice_detail->getRice_name(); ?>" required/>  
                                                </div>
                                                <br>
                                                <div class="form-group my-2">
                                                    <label for="rice_price">Rice Price :</label>
                                                    <input style="background-color: black; color: white;" name="rice_price" type="number" id="rice_price" class="form-control" value="<?php echo $rice_detail->getRice_price(); ?>" required/>
                                                </div>
                                                <br>
                                                <br>
                                                <button type="submit" name="submit" class="mt-3 btn btn-lg btn-block form-control" style="background-color: #333; color: white; border: 1px solid #E88F2A;">UPDATE RICE</button>
                                            </div>
                                        </div>
                                    </div>
                                </form>
                    ...
                    echo "Error in Update Rice details: " . $exc->getMessage();
                }
                ?>
